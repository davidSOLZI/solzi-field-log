...
// ── Movement categories ───────────────────────────────────────────────────
const CATEGORIES = [
  { label: "Sampling",  sheetLocation: "Marketing", items: ["Retail MAC", "Events", "Retail Pop-Up"] },
  { label: "Marketing", sheetLocation: "Marketing", items: ["Influencer Gifting", "Strategic Seeding", "UGC / Content", "Photoshoot", "Founder Use"] },
  { label: "Retail",    sheetLocation: "Retail",    items: ["First Case Free", "Retail Order"] },
  { label: "Customer",  sheetLocation: "Customer",  items: ["Offices", "Manual DTC Order"] },
  { label: "Shrinkage", sheetLocation: "Loss",      items: ["Damaged / Lost", "Adjustment"] },
];

// ── Shared styles ─────────────────────────────────────────────────────────
const label11 = { fontSize: 10, fontWeight: 700, letterSpacing: 1.2, color: "#8B7355", textTransform: "uppercase", marginBottom: 8, paddingLeft: 2 };
const card    = { background: CARD, borderRadius: 12, border: `1px solid ${BORDER}`, marginBottom: 16 };
const inputSt = { padding: "8px 10px", borderRadius: 8, border: `1px solid ${BORDER}`, background: "#fff", fontSize: 14, color: DARK, outline: "none", width: "100%", boxSizing: "border-box", fontFamily: "inherit" };

// ── Toast ─────────────────────────────────────────────────────────────────
function Toast({ msg, type }) {
  if (!msg) return null;
  return (
    <div style={{ position: "fixed", bottom: 90, left: "50%", transform: "translateX(-50%)", background: type === "success" ? "#2E7D32" : "#B71C1C", color: "#fff", borderRadius: 10, padding: "10px 22px", fontSize: 14, fontWeight: 600, zIndex: 999, boxShadow: "0 4px 20px rgba(0,0,0,0.25)", whiteSpace: "nowrap" }}>{msg}</div>
  );
}
...
function FieldLogTab({ onSuccess }) {
  ...
  async function handleSubmit() {
    ...
    try {
      await fetch(APPS_SCRIPT_URL, {
        ...
        body: JSON.stringify({ action: "fieldLog", movementType, cases, cans: loose, totalCans, notes }),
      });
      setStatus("success");
  ...
  return (
    <div style={{ padding: "16px 16px 100px" }}>
      <div style={label11}>Movement Type</div>
          ...
          <div key={cat.label}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: cat.items.length === 1 ? "1fr" : "1fr 1fr", gap: 8, padding: "8px 10px 10px" }}>
              {cat.items.map((item, idx) => {
                ...
                return (
                  <button key={item} onClick={() => setMovementType(item)} style={{ padding: "10px 6px", borderRadius: 8, border: movementType === item ? `2px solid ${TERRA}` : `1px solid ${BORDER}`, background: movementType === item ? "#FDE8DE" : "#fff", color: movementType === item ? TERRA : DARK, fontWeight: movementType === item ? 700 : 400, fontSize: 13, cursor: "pointer", transition: "all 0.12s", gridColumn: isLast && isOdd ? "1 / -1" : undefined }}>{item}</button>
                );
              })}
            </div>
          </div>
      ...
      <div style={{ ...card, padding: "16px 12px" }}>
        ...
        <div style={{ marginTop: 14, background: totalCans > 0 ? DARK : "#E8DDD0", borderRadius: 8, padding: "10px 16px", display: "flex", justifyContent: "space-between", alignItems: "center", transition: "background 0.2s" }}>
      ...
      <div style={label11}>Notes (Optional)</div>
      <textarea value={notes} onChange={e => setNotes(e.target.value)} placeholder="e.g. Venice pop-up..." rows={2} style={{ ...inputSt, resize: "none", marginBottom: 20 }} />

      <button onClick={handleSubmit} disabled={!ready || status === "loading"} style={{ width: "100%", padding: "14px", borderRadius: 12, border: "none", background: status === "success" ? "#2E7D32" : ready ? TERRA : "#C8B8A2", color: "#fff", fontSize: 16, fontWeight: 700, cursor: ready ? "pointer" : "default", transition: "background 0.3s" }}>
...
function InventoryCountTab() {
  ...
  const [status, setStatus]           = useState("idle");
  const [toast, setToast]             = useState(null);
  const [ledgerCases, setLedgerCases] = useState(null);   // from Apps Script
  const [ledgerLoading, setLedgerLoading] = useState(true);

  // Fetch ledger Airdrome value on mount
  useEffect(() => {
  ...
  function totalCasesVal() { return parseFloat((totalCans() / 12).toFixed(2)); }

  // Gap vs ledger (in cases, + = you have more than ledger, - = short)
  const gapCases = ledgerCases !== null ? parseFloat((totalCasesVal() - ledgerCases).toFixed(2)) : null;
  // Color coding: green ≤ 1 case, amber ≤ 3, red > 3
  const gapColor = gapCases === null ? "#8B7355"
    : Math.abs(gapCases) <= 1  ? "#2E7D32"
    : Math.abs(gapCases) <= 3  ? "#F57F17"
    : "#B71C1C";

  const updateFixed = (loc, field, val) => setLocs(prev => ({ ...prev, [loc]: { ...prev[loc], [field]: val } }));
  const updateExtra = (idx, field, val) => setExtraLocs(prev => prev.map((l, i) => i === idx ? { ...l, [field]: val } : l));
  const addLoc = () => { const n = newLocName.trim(); if (!n) return; setExtraLocs(prev => [...prev, { name: n, cases: "", cans: "" }]); setNewLocName(""); };
  const showToast = (msg, type) => { setToast({ msg, type }); setTimeout(() => setToast(null), 3500); };
  ...
  async function handleSubmit() {
    ...
    const weekStart = new Date(now); const wd = now.getDay(); weekStart.setDate(now.getDate() - wd + (wd === 0 ? -6 : 1));
    ...
    try {
      await fetch(APPS_SCRIPT_URL, { method: "POST", mode: "no-cors", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ action: "inventoryCount", date: fmt(now), time: now.toLocaleTimeString("en-US", { hour: "numeric", minute: "2-digit" }), weekOf: `${fmt(weekStart)} – ${fmt(weekEnd)}`, totalCases: totalCasesVal(), totalCans: totalCans(), ledgerExpected: ledgerCases !== null ? ledgerCases * 12 : "", variance: gapCases !== null ? Math.round(gapCases * 12) : "", locations: JSON.stringify(allLocs), notes }) });
  ...
  const smInput = { ...inputSt, padding: "7px 8px", fontSize: 13, borderRadius: 7 };
  ...
  function LocRow({ label, data, onUpdate }) {
    ...
    return (
      <div style={{ display: "grid", gridTemplateColumns: "minmax(90px,1fr) 78px 78px 48px", gap: 6, alignItems: "center", padding: "9px 0", borderBottom: `1px solid ${BORDER}` }}>
        ...
        <input type="number" min="0" step="0.5" placeholder="Cases" value={data.cases} onChange={e => onUpdate("cases", e.target.value)} style={smInput} />
        <input type="number" min="0" placeholder="Cans"  value={data.cans}  onChange={e => onUpdate("cans",  e.target.value)} style={smInput} />
        <span style={{ fontSize: 12, color: "#8B7355", textAlign: "right", fontWeight: 600 }}>{rowCans > 0 ? rowCans : "—"}</span>
      </div>
    );
  }
  ...
  return (
    <div style={{ padding: "16px 16px 100px" }}>
      <div style={label11}>Weekly Count</div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "minmax(90px,1fr) 78px 78px 48px", gap: 6, paddingBottom: 6 }}>
        {["Location","Cases","Cans","Total"].map((h,i) => (
          <span key={h} style={{ fontSize: 10, color: "#8B7355", fontWeight: 700, textTransform: "uppercase", letterSpacing: 0.8, textAlign: i===3 ? "right" : "left" }}>{h}</span>
        ))}
      </div>

      <div style={{ ...card, padding: "0 14px" }}>
        {FIXED_LOCS.map(loc => <LocRow key={loc} label={loc} data={locs[loc]} onUpdate={(f,v) => updateFixed(loc,f,v)} />)}
        {extraLocs.map((loc,i) => <LocRow key={loc.name} label={loc.name} data={loc} onUpdate={(f,v) => updateExtra(i,f,v)} />)}
        <div style={{ display: "flex", gap: 8, padding: "10px 0" }}>
          <input type="text" placeholder="+ Add location" value={newLocName} onChange={e => setNewLocName(e.target.value)} onKeyDown={e => e.key === "Enter" && addLoc()} style={{ ...smInput, flex: 1 }} />
      ...
      <div style={{ background: DARK, borderRadius: 12, padding: "14px 16px", marginBottom: 16 }}>
        ...
        <div style={{ borderTop: "1px solid #4a3020", paddingTop: 10, marginTop: 2 }}>
          <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 6 }}>
            <span style={{ color: GOLD, fontSize: 13 }}>Ledger (Airdrome)</span>
            <span style={{ color: "#aaa", fontSize: 14, fontWeight: 600 }}>
              {ledgerLoading ? "…" : ledgerCases !== null ? `${ledgerCases} cases` : "—"}
            </span>
          </div>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
            <span style={{ color: GOLD, fontSize: 13 }}>Gap</span>
            <span style={{ fontSize: 18, fontWeight: 800, color: gapColor }}>
              {gapCases === null ? "—"
                : gapCases === 0 ? "✓ Even"
                : `${gapCases > 0 ? "+" : ""}${gapCases} cases`}
            </span>
          </div>
          {gapCases !== null && Math.abs(gapCases) > 0 && (
            <div style={{ marginTop: 6, padding: "6px 10px", borderRadius: 7, background: gapColor + "22", textAlign: "center" }}>
              <span style={{ color: gapColor, fontSize: 12, fontWeight: 600 }}>
                {Math.abs(gapCases) <= 1 ? "Within normal range" : Math.abs(gapCases) <= 3 ? "Minor discrepancy — worth reviewing" : "Significant gap — investigate"}
              </span>
            </div>
          )}
        </div>
      ...
      <div style={label11}>Notes (Optional)</div>
      <textarea value={notes} onChange={e => setNotes(e.target.value)} placeholder="Any notes about this count..." rows={2} style={{ ...inputSt, resize: "none", marginBottom: 20 }} />
...
function DashboardTab({ refreshKey }) {
  ...
  const totalUsageCases = (weeklyUsage.Customer || 0) + (weeklyUsage.Retail || 0) + (weeklyUsage.Marketing || 0);

  return (
    <div style={{ padding: "16px 16px 100px" }}>

      {/* On hand */}
      <div style={label11}>Total On Hand</div>
      <div style={{ display: "flex", gap: 10, marginBottom: 16 }}>
        {[
          ...
          <div key={label} style={{ ...card, flex: 1, marginBottom: 0, padding: "12px 14px", background: accent ? DARK : CARD }}>
      ...
      {/* Usage by category with % */}
      <div style={label11}>Usage — All Time</div>
      <div style={{ ...card, padding: 0, overflow: "hidden", marginBottom: 16 }}>
          ...
          <div key={label} style={{ padding: "11px 16px", borderBottom: i < arr.length - 1 ? `1px solid ${BORDER}` : "none" }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 5 }}>
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                ...
                <span style={{ fontSize: 15, fontWeight: 700, color: TERRA, minWidth: 40, textAlign: "right" }}>{Math.round(value)} cs</span>
              </div>
            </div>
            {/* Progress bar */}
            <div style={{ height: 4, background: BORDER, borderRadius: 4, overflow: "hidden" }}>
              <div style={{ height: "100%", width: `${pct}%`, background: TERRA, borderRadius: 4, transition: "width 0.6s ease" }} />
            </div>
          </div>
        ))}
        <div style={{ padding: "10px 16px", background: "#F9F4EE", display: "flex", justifyContent: "space-between" }}>
          ...
          <span style={{ fontSize: 13, fontWeight: 700, color: DARK }}>{Math.round(totalUsageCases)} cases</span>
        </div>
      </div>

      {/* This week */}
      <div style={label11}>This Week (Field Log)</div>
      <div style={{ ...card, padding: "12px 16px", display: "flex", gap: 16, marginBottom: 16 }}>
      ...
      {/* Recent entries */}
      <div style={label11}>Recent Entries</div>
      <div style={{ ...card, overflow: "hidden" }}>
...
export default function App() {
  ...
  return (
    <div style={{ minHeight: "100vh", background: CREAM, fontFamily: "-apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif" }}>
      {/* Fixed header */}
      <div style={{ position: "fixed", top: 0, left: 0, right: 0, zIndex: 100, background: TERRA, boxShadow: "0 2px 12px rgba(0,0,0,0.18)" }}>
        ...
        <div style={{ display: "flex" }}>
          {TABS.map(t => (
            <button key={t.id} onClick={() => setTab(t.id)} style={{ flex: 1, padding: "8px 4px 9px", border: "none", background: "transparent", color: tab === t.id ? "#fff" : "rgba(255,255,255,0.6)", fontWeight: tab === t.id ? 700 : 400, fontSize: 12, cursor: "pointer", borderBottom: tab === t.id ? "2.5px solid #fff" : "2.5px solid transparent", transition: "all 0.15s", fontFamily: "inherit" }}>{t.label}</button>
          ))}
        </div>
      </div>
